...
export default function Create() {
  ...
  const [form, setForm] = useState({ child_name: "", age: 5, gender: "", theme: "Moonlit Forest", visual_style: "Classic Watercolor", photo_base64: "", story_language: "en", story_prompt: "", page_count: 24, voice_id: 'nova' });
  ...
  const onPhoto = async event => {
    ...
    try { const dataUrl = await readAsDataUrl(file); setPhotoName(file.name); setError(''); setForm(previous => ({ ...previous, photo_base64: dataUrl })); }
    catch { setError(language === 'id' ? 'Foto belum bisa dibaca.' : 'This photo could not be read.'); }
  };

  const submit = async event => {
    event.preventDefault();
    if (!loading) startGeneration(form);
  };
  ...
  const startGeneration = async payload => {
    ...
    try {
      if (!user) {
        ...
        if (data.story_id) { navigate(`/storybook/${data.story_id}`); return; }
      }
    ...
    <Shell>
      <main className="create-layout">
        ...
        <form className="form-panel" onSubmit={submit}>
          ...
          {!user && <small className="guest-limit-note" data-testid="guest-story-limit-note">{language === 'id' ? 'Satu cerita gratis sebagai tamu. ' : 'One story as a guest. '}<Link to="/signup" data-testid="create-signup-link">{language === 'id' ? 'Buat akun untuk menyimpan cerita.' : 'Create an account to keep your stories.'}</Link></small>}
