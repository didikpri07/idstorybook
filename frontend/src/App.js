...
function Create() {
  ...
  const submit = async event => { event.preventDefault(); setLoading(true); setError(""); try { const response = await axios.post(`${API}/stories`, form, { timeout: 180000 }); setPreview(response.data.id); } catch (_) { setError(text.createError); } finally { setLoading(false); } };
